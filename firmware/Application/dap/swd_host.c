...
uint8_t swd_flash_syscall_exec (
    const program_syscall_t *sysCallParam, uint32_t entry, uint32_t arg1, uint32_t arg2, uint32_t arg3, uint32_t arg4) {
...
uint8_t swd_fetch_init (void) {
    uint32_t tmp = 0;

    /* 清错误标志 */
    swd_write_dp(DP_ABORT, STKCMPCLR | STKERRCLR | WDERRCLR | ORUNERRCLR);

    /* SELECT 归零 */
    swd_write_dp(DP_SELECT, 0);

    /* 系统电源上电 */
    swd_write_dp(DP_CTRL_STAT, CSYSPWRUPREQ);
    do {
        swd_read_dp(DP_CTRL_STAT, &tmp);
    } while ((tmp & CSYSPWRUPACK) == 0);

    /* 调试电源上电 */
    swd_write_dp(DP_CTRL_STAT, CSYSPWRUPREQ | CDBGPWRUPREQ);
    do {
        swd_read_dp(DP_CTRL_STAT, &tmp);
    } while ((tmp & (CDBGPWRUPACK | CSYSPWRUPACK)) !=
...
void swd_set_target_reset (uint8_t asserted) {
    /* 本文件中对此函数的使用都是先 asserted=1 调用，延时后 asserted=0
     * 调用，为了只调用一次所以只在第二次调用此函数时执行软件复位 */
    if (asserted == 0) {
        // swd_write_word(
        //     (uint32_t)&SCB->AIRCR,
        //     ((0x5FA << SCB_AIRCR_VECTKEY_Pos) | (SCB->AIRCR & SCB_AIRCR_PRIGROUP_Msk) | SCB_AIRCR_SYSRESETREQ_Msk)
        // );
    }
}
...
uint8_t swd_set_target_state_hw (TARGET_RESET_STATE state) {
    ...
    switch (state) {
    case RESET_HOLD: swd_set_target_reset (1); break;
